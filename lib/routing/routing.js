...
function getControllers(parent_route){
    var namespace = "";
    if(parent_route.namespace){
        namespace = parent_route.namespace;
    }

    var array_length = parent_route['childRoutes'].length;
    for(var i=0; i<array_length;i++){
        controllers.push(namespace+"/"+parent_route['childRoutes'][i].controller);
    }
}
